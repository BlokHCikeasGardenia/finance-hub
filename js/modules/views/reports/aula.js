// Aula (Hall) Transaction Reports Module
// Hall rental and service transaction reports

import { supabase } from '../../config.js';
import { showToast, formatCurrency } from '../../utils.js';

// Global state for Rekap Aula
let aulaRekapDataGlobal = [];
...
async function loadViewAula() {
    ...
    try {
        // First, get the Aula category ID
        const { data: aulaCategory, error: categoryError } = await supabase
            .from('kategori_saldo')
            .select('id')
            .eq('nama_kategori', 'Aula')
            .single();

        if (categoryError || !aulaCategory) {
            console.error('Aula category not found:', categoryError);
            contentDiv.innerHTML = '<p class="text-danger">Kategori Aula tidak ditemukan. Pastikan data master kategori sudah diisi.</p>';
            return;
        }

        // Get all aula-related transactions (both income and expenses)
        const aulaTransactions = [];

        // Get pemasukan (income) for aula
        const { data: pemasukanData, error: pemasukanError } = await supabase
            ...
            .select(`
                ...
                keterangan
            `)
            .eq('kategori_id', aulaCategory.id)
        ...
        // Get pengeluaran (expenses) for aula
        ...
        aulaTransactions.sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal));

        const html = `
            <div class="row">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h4>View Data Aula</h4>
                            <button class="btn btn-warning text-dark" onclick="loadViewsSection()">
                                <i class="bi bi-arrow-left"></i> Back
                            </button>
                    </div>
                    <p class="text-muted">Data transaksi pemasukan dan pengeluaran terkait aula</p>
                    ...
                    ${aulaTransactions.length === 0 ? '<p class="text-muted">Belum ada transaksi aula.</p>' : ''}

                    <div class="mt-3">
                        <button class="btn btn-outline-primary" onclick="refreshViewAula()">
        ...
        console.error('Error loading aula view:', error);
        contentDiv.innerHTML = '<p class="text-danger">Error loading aula data</p>';
    }
}
...
async function loadViewRekapAula(selectedYear = null) {
    ...
    try {
        ...
        const infoText = isAllYearsMode
            ? 'Rekap pemasukan dan pengeluaran aula dari semua periode'
            : `Rekap pemasukan dan pengeluaran aula tahun ${displayYear} per periode`;
        ...
        const html = `
            <div class="row">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="d-flex gap-2 align-items-center">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <label for="rekap-aula-year-select" class="form-label mb-0 fw-bold">Filter Tahun:</label>
                                <select class="${selectorClass}" id="rekap-aula-year-select" style="width: auto;">
                    ...
                    <p class="text-muted">Rekap pemasukan dan pengeluaran aula per periode</p>
                            ...
                            <tbody>
                                ${rekapData.map((item, index) => `
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            ${item.pemasukan > 0 ?
                                                `<button class="btn btn-link text-success fw-bold p-0" onclick="showRekapAulaPemasukanDetails(${index})">${formatCurrency(item.pemasukan)}</button>` :
                                                formatCurrency(item.pemasukan)}
                                        </td>
                                        <td class="text-end">
                                            ${item.pengeluaran > 0 ?
                                                `<button class="btn btn-link text-danger fw-bold p-0" onclick="showRekapAulaPengeluaranDetails(${index})">${formatCurrency(item.pengeluaran)}</button>` :
                    ...
                    <div class="mt-3">
                        <button class="btn btn-outline-primary" onclick="refreshViewRekapAula()">
                            <i class="bi bi-arrow-clockwise"></i> Refresh Data
                        </button>
                    </div>
                </div>
        ...
        aulaRekapDataGlobal = rekapData;

        contentDiv.innerHTML = html;

        // Initialize year selector functionality
        setTimeout(() => {
            initializeRekapAulaYearSelector();
        }, 100);
    } catch (error) {
        console.error('Error loading rekap aula view:', error);
        contentDiv.innerHTML = '<p class="text-danger">Error loading rekap aula data</p>';
    }
}

// Initialize Rekap Aula Year Selector
function initializeRekapAulaYearSelector() {
    const yearSelect = document.getElementById('rekap-aula-year-select');
    if (yearSelect) {
        yearSelect.addEventListener('change', (e) => {
            const selectedYear = e.target.value;
            loadViewRekapAula(selectedYear);
        });
    }
}

// Refresh Rekap Aula View
async function refreshViewRekapAula() {
    const yearSelect = document.getElementById('rekap-aula-year-select');
    const selectedYear = yearSelect ? yearSelect.value : null;
    await loadViewRekapAula(selectedYear);
}

// Refresh Aula View
async function refreshViewAula() {
    await loadViewAula();
}
...
export {
    ...
    refreshViewAula,
    loadViewRekapAula,
    refreshViewRekapAula,
    initializeRekapAulaYearSelector
};

// Show Rekap Aula Pemasukan Details
function showRekapAulaPemasukanDetails(periodIndex) {
    const periodData = aulaRekapDataGlobal[periodIndex];
    ...
    const detailsHtml = `
        <div class="modal fade" id="rekapAulaPemasukanModal" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Detail Pemasukan Aula - ${periodData.periode}</h5>
    ...
    const existingModal = document.getElementById('rekapAulaPemasukanModal');
    if (existingModal) {
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('rekapAulaPemasukanModal'));
    modal.show();
}

// Show Rekap Aula Pengeluaran Details
function showRekapAulaPengeluaranDetails(periodIndex) {
    const periodData = aulaRekapDataGlobal[periodIndex];
    ...
    const detailsHtml = `
        <div class="modal fade" id="rekapAulaPengeluaranModal" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Detail Pengeluaran Aula - ${periodData.periode}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="table-responsive">
    ...
    // Remove existing modal if any
    const existingModal = document.getElementById('rekapAulaPengeluaranModal');
    if (existingModal) {
        existingModal.remove();
    }
    ...
    // Show modal
    const modal = new bootstrap.Modal(document.getElementById('rekapAulaPengeluaranModal'));
    modal.show();
}

// Backward compatibility for global window functions
window.loadViewAula = loadViewAula;
window.refreshViewAula = refreshViewAula;
window.loadViewRekapAula = loadViewRekapAula;
window.refreshViewRekapAula = refreshViewRekapAula;
window.showRekapAulaPemasukanDetails = showRekapAulaPemasukanDetails;
window.showRekapAulaPengeluaranDetails = showRekapAulaPengeluaranDetails;
